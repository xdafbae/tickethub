...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $ordersPaid = $ordersPaidQuery->with('event')->get();
        ...
        return view('admin.dashboard', compact(
            'from','to','eventId','events',
            'netRevenue','ordersCountPaid','ticketsSold',
            'perEvent','dailyLabels','dailyNetRevenue'
        ));
    }
    ...
    public function export(Request $request, string $format)
    {
        ...
        if (!in_array($format, ['csv', 'xls'])) abort(400, 'Format tidak didukung.');
        ...
        foreach ($rows as $row) {
            $format === 'csv' ? fputcsv($handle, $row) : fwrite($handle, implode("\t", $row)."\n");
        }
        rewind($handle);
