...
@section('page-title', 'Tambah Tipe Tiket')
...
<div class="table-card" style="max-width: 600px;">
    <form action="{{ route('admin.ticket_types.store') }}" method="POST" class="form-event">
        @csrf
        <div style="padding: 30px; display: flex; flex-direction: column; gap: 20px;">
            <div class="form-group">
                ...
                <select name="event_id" required class="form-input">
                    ...
                    @foreach($events as $ev)
                        <option value="{{ $ev->id }}" {{ (old('event_id') ?? request('event_id')) == $ev->id ? 'selected' : '' }}>{{ $ev->title }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" id="priceDisplay" name="price_view" placeholder="Rp 0" value="{{ old('price') ? number_format((int)old('price'),0,',','.') : '' }}" class="form-input">
                <input type="hidden" name="price" id="priceRaw" value="{{ old('price') }}">
            ...
            <div class="form-group">
                ...
                <input type="number" name="quota" required value="{{ old('quota') }}" class="form-input">
            ...
            <div class="form-group" style="display:grid; grid-template-columns: 1fr 1fr; gap:12px; align-items:center;">
                <div>
                    ...
                    <input type="date" name="available_from" value="{{ old('available_from') }}" class="form-input">
                </div>
                <div>
                    <label>hingga</label>
                    <input type="date" name="available_to" value="{{ old('available_to') }}" class="form-input">
                </div>
            </div>
...
<script>
    (function(){
        var form = document.querySelector('form[action="{{ route('admin.ticket_types.store') }}"]');
