...
@section('content')
    <div class="container" style="max-width: 1100px; margin: 0 auto; padding: 16px;">
        <div
            style="border:1px solid var(--admin-border); border-radius:16px; background: rgba(255,255,255,0.06); overflow:hidden;">
                ...
                <div style="flex:0 0 360px; border-radius:12px; overflow:hidden; background:#0f0f10;">
                    ...
                    @if ($event->poster)
                        <img src="{{ asset('storage/' . $event->poster) }}" alt="{{ $event->title }}"
                            style="width:100%; height:100%; max-height: 320px; object-fit:cover;">
                    @else
                        <img src="https://picsum.photos/seed/{{ $event->id }}/800/480" alt="{{ $event->title }}"
                            style="width:100%; height:100%; max-height: 320px; object-fit:cover;">
                    @endif
                </div>
                ...
                <div style="flex:1 1 520px; min-width:280px;">
                    ...
                    <div style="display:flex; gap:10px; align-items:center; margin-bottom:12px; flex-wrap:wrap;">
                        ...
                        <span style="color: var(--admin-muted);">📅
                            {{ $event->date?->locale('id')->translatedFormat('d M Y') }}</span>
                    ...
                    @endphp
                    <div
                        style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 12px; margin: 14px 0 18px; align-items:stretch;">
                        <div
                            style="border:1px solid var(--admin-border); border-radius:10px; padding:12px; background: rgba(255,255,255,0.04); box-sizing:border-box;">
                            <div style="color: var(--admin-muted); font-size:12px;">Tanggal</div>
                            <div style="font-weight:700; word-break:break-word;">
                                {{ $event->date?->locale('id')->translatedFormat('l, d M Y') }}</div>
                        </div>
                        <div
                            style="border:1px solid var(--admin-border); border-radius:10px; padding:12px; background: rgba(255,255,255,0.04); box-sizing:border-box;">
                    ...
                    @if ($event->description)
                        <div
                            style="margin-top:12px; border:1px solid var(--admin-border); border-radius:10px; padding:12px; background: rgba(255,255,255,0.04); box-sizing:border-box;">
                            <h3 style="margin: 0 0 8px; font-size: 16px;">Deskripsi</h3>
                            <div
                                style="color: var(--admin-muted); line-height:1.6; white-space: normal; overflow-wrap:anywhere; word-break:break-word;">
                    ...
                    @if ($types->count() > 0)
                        <div
                            style="margin-top:16px; border:1px solid var(--admin-border); border-radius:10px; padding:12px; background: rgba(255,255,255,0.04); box-sizing:border-box;">
                            <h3 style="margin: 0 0 8px; font-size: 16px;">Tipe Tiket</h3>
                            <div
                                style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 10px; align-items:stretch;">
                                @foreach ($types as $t)
                                    @php
                                    ...
                                    @endphp
                                    <div
                                        style="border:1px solid var(--admin-border); border-radius:10px; padding:10px; background: rgba(255,255,255,0.04); box-sizing:border-box; border-left:4px solid {{ $seatColor }};">
                                        <div
                                        ...
                                        @if ($t->description)
                                            <div
                                                style="margin-top:6px; color: var(--admin-muted); font-size:13px; white-space: normal; overflow-wrap:anywhere; word-break:break-word;">
                                                {{ $t->description }}
                                            </div>
                                        @endif
                                    </div>
                    ...
                    <div style="margin-top:20px;">
                        ...
                        <div id="selectMapContainer"
                            ...
                            <svg id="selectMapSvg" width="100%" height="100%" style="display:block;">
                                <g id="selectMapContent" transform="scale(1)" data-scale="1" style="transform-origin: 0 0;">
                                </g>
                                <rect id="lasso" x="0" y="0" width="0" height="0" fill="rgba(59,130,246,0.2)"
                                    stroke="#3b82f6" stroke-dasharray="4 3" style="display:none;"></rect>
                            </svg>
                        </div>

                        {{-- Toolbar Selected diposisikan di luar seat selection, tepat di bawahnya --}}
                        <div id="selectToolbar"
                            style="margin-top:12px; border:1px solid var(--admin-border); border-radius:8px; padding:10px 12px; display:flex; justify-content:space-between; align-items:center; gap:8px; background: rgba(0,0,0,0.25);">
                    ...
                    <script>
                        (function() {
                            ...
                            // peta seat -> ticket type
                            var typeIdBySeatId = {};
                            var typeNameBySeatId = {};
                            ...
                            function updateSelectedList() {
                                var arr = Array.from(selected).map(function(id) {
                                    return nameById[id] || id;
                                });
                                document.getElementById('selectedList').textContent = arr.length ? arr.join(', ') : '—';
                                var total = 0;
                                selected.forEach(function(id) {
                                    var ttId = typeIdBySeatId[id];
                                    var price = 0;
                                    if (ttId != null && priceByTypeId[ttId] != null) {
                                        price = Number(priceByTypeId[ttId]);
                                    } else {
                                        var ttName = typeNameBySeatId[id];
                                        var p = ttName ? priceByTypeName[ttName] : null;
                                        price = p != null ? Number(p) : 0;
                                    }
                                    total += price;
                                });
                                document.getElementById('selectedTotal').textContent = 'Rp ' + formatId(total);
                            }
                            ...
                            function draw(layout) {
                                ...
                                var chairs = layout.filter(function(n) {
                                    return n.type === 'chair';
                                });
                                var others = layout.filter(function(n) {
                                    return n.type !== 'chair';
                                });
                                ...
                                others.forEach(function(n) {
                                    ...
                                    rect.setAttribute('fill', (n.type === 'stage' || n.type === 'talent') ? '#9ca3af' :
                                        '#1f2937');
                            ...
                            function fetchMap() {
                                fetch('{{ route('user.events.seat.map', $event) }}', {
                                        headers: {
                            ...
                            svg.addEventListener('mousemove', function(e) {
                                ...
                                lasso.setAttribute('width', w);
                                lasso.setAttribute('height', h);
                            });
                            ...
                            // Zoom tombol
                            document.getElementById('selZoomIn').addEventListener('click', function() {
                                scale = Math.min(MAX, scale + STEP);
                                applyScale();
                            });
                            document.getElementById('selZoomOut').addEventListener('click', function() {
                                scale = Math.max(MIN, scale - STEP);
                                applyScale();
                            });
                            document.getElementById('selZoomReset').addEventListener('click', function() {
                                scale = 1;
                                applyScale();
                            });
